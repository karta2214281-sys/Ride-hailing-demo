...
class TaxiSystem {
    ...
    initializeEventListeners() {
        ...
        if (toggleBtn) {
            ...
            console.error('找不到 toggleClearBtn 元素');
        }
    ...
    handleDriverMessage() {
        ...
        if (this.roleMode === 'driver') {
            ...
            } else if (message.includes('抵達') && this.currentOrder && this.currentOrder.assignedDriver) {
                // 檢查抵達確認
                this.processDriverArrival(message);
            } else {
                // 一般訊息
    ...
    updateCountdownDisplay() {
        ...
        if (this.isWaitingForDriver && this.countdownSeconds > 0) {
            ...
            countdownControls.style.display = 'inline-flex';
        } else {
    ...
    processDriverBid(message) {
        ...
        // 檢查車輛顏色是否符合特殊需求
        const specialRequirements = this.currentOrder.specialRequirements;
        let colorConflict = false;
        let conflictMessage = '';
        
        if (specialRequirements.includes('不要黑色車') && color === '黑') {
            colorConflict = true;
            conflictMessage = '不要黑色車';
        } else if (specialRequirements.includes('不要白色車') && color === '白') {
            colorConflict = true;
            conflictMessage = '不要白色車';
        } else if (specialRequirements.includes('不要紅色車') && color === '紅') {
            colorConflict = true;
            conflictMessage = '不要紅色車';
        } else if (specialRequirements.includes('不要藍色車') && color === '藍') {
            colorConflict = true;
            conflictMessage = '不要藍色車';
        }
        
        if (colorConflict) {
            // 延遲一點時間再回應，讓使用者看到司機的訊息
            setTimeout(() => {
                this.addMessage('C', '白牌叫車', `@${this.driverName} 抱歉，您的車輛顏色不符合客戶需求（客戶要求${conflictMessage}）`, 'system');
            }, 500);
            return;
        }
    ...
    checkDriverBids() {
        ...
        const validBids = this.driverBids.filter(bid => {
            ...
            if (specialRequirements.includes('不要黑色車') && bid.color === '黑') {
                return false;
            } else if (specialRequirements.includes('不要白色車') && bid.color === '白') {
                return false;
            } else if (specialRequirements.includes('不要紅色車') && bid.color === '紅') {
                return false;
            } else if (specialRequirements.includes('不要藍色車') && bid.color === '藍') {
                return false;
            }
    ...
    sendArrivalNotification() {
        ...
        this.addMessageGroup('B', '白牌叫車', [
            ...
            `群組：${order.group}\n` +
            `乘客：${order.customer}`,
            `@${order.customer} 車輛已抵達 ${order.address}`
        ], 'system');
        ...
        // 更新系統狀態
        document.getElementById('systemStatus').textContent = '司機已抵達';
        document.getElementById('systemStatus').className = 'status-arrived';
    }
    ...
    toggleClearPanel() {
        ...
        console.log('按鈕被點擊了'); // 除錯用
        console.log('面板當前顯示狀態:', panel.style.display);
    ...
    quickAddOrder() {
        // 1. 自動在A群組用潘潘輸入叫車訊息
        const customerMessage = '@白牌叫車 台南火車站 不要黑色車';
        this.addMessage('A', '潘潘', customerMessage, 'customer');
        
        // 2. 系統自動回應
        setTimeout(() => {
            this.addMessage('A', '白牌叫車', '@潘潘 馬上為您安排', 'system');
        }, 500);
        ...
        setTimeout(() => {
            this.currentOrder = {
                id: this.currentOrderNumber,
                address: '台南火車站',
                specialRequirements: '不要黑色車',
                customer: '潘潘',
                group: 'AA'
            };
            
            // B群組顯示訂單詳情（使用 addMessageGroup 形成一個群組）
            this.addMessageGroup('B', '白牌叫車', [
                '新單通知！',
                '台南火車站',
                `訂單編號：${this.currentOrderNumber}\n` +
                `地址：台南火車站\n` +
                `特殊需求：不要黑色車\n` +
                `群組：AA\n` +
                `乘客：潘潘`
        ...
        // 4. 自動在C群組後台人員模式貼上通知
        setTimeout(() => {
            this.addMessage('C', '服務人員', '新單通知!', 'admin');
            this.addMessage('C', '服務人員', '台南火車站', 'admin');
        }, 1500);
        ...
        setTimeout(() => {
            ...
            // 更新系統狀態
            document.getElementById('systemStatus').textContent = '等待司機搶單';
            document.getElementById('systemStatus').className = 'status-waiting';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 測試按鈕是否可用
    const testBtn = document.getElementById('toggleClearBtn');
    if (testBtn) {
        console.log('找到按鈕元素');
        // 添加簡單的測試事件
